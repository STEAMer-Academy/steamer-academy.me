...
export default function About() {
  ...
  return (
    <div className={`space-y-16 ${$theme === 'dark' ? 'bg-[#1a1b26] text-[#a9b1d6]' : 'bg-white text-gray-900'}`}>
      <section>
        <motion.div
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8 p-10">
          {teamMembers.map((member, index) => (
            <motion.div
              ...
              viewport={{ once: true }}
            >
              <Card className={`${$theme === 'dark' ? 'bg-gray-800 hover:bg-gray-700' : 'bg-white hover:bg-gray-100'} p-7 rounded-lg shadow-lg transition-transform transform hover:scale-105 hover:shadow-2xl`}>
                <CardHeader>
                ...
                <CardContent className="text-center p-12 pb-4">
                  <CardHeader className={`text-xl font-semibold ${$theme === 'dark' ? 'text-white' : 'text-gray-900'} mb-2 p-1`}>{member.name}</CardHeader>
                  <CardTitle className={`text-lg ${$theme === 'dark' ? 'text-gray-400' : 'text-gray-600'} mb-4 p-4`}>{member.role}</CardTitle>
                  <CardDescription className={`font-medium ${$theme === 'dark' ? 'text-gray-300' : 'text-gray-700'}`}>{member.content}</CardDescription>
                </CardContent>
              </Card>
            </motion.div>
          ))}
